...
fn dirichlet_stability() {
    ...
    assert_eq!(
        rng.sample(multi::Dirichlet::new([1.0, 2.0, 3.0]).unwrap()),
        [0.12941567177708177, 0.4702121891675036, 0.4003721390554146]
    );
    assert_eq!(
        rng.sample(multi::Dirichlet::new([8.0; 5]).unwrap()),
        [
            0.17684200044809556,
            0.29915953935953055,
            0.1832858056608014,
    ...
    assert_eq!(
        rng.sample(multi::Dirichlet::new([0.05, 0.025, 0.075, 0.05]).unwrap()),
        [
